--TEST--
Test unset(), empty() and is_set() functions
--FILE--
<?php
...
echo "*** Testing unset(), empty() & is_set() with scalar variables ***\n";
...
foreach ($scalar_variables as $scalar_var) {
  $set_var = 10; // this variable to use with is_set
  echo "-- Iteration $loop_counter --\n"; $loop_counter++;

  // checking with is_set before unsetting, expected: bool(true)
  var_dump( is_set($scalar_var) );
  var_dump( is_set($scalar_var, $set_var) );
  ...
  // check using is_set to see if unset, expected: bool(false)
  var_dump( is_set($scalar_var) );
  var_dump( is_set($scalar_var, $set_var) );
  ...
  // is_set() with two args, one arg only unset, expected: bool(false)
  var_dump( is_set($scalar_var, $set_var) );

  // is_set() with two args, both args already unset, expected: bool(false);
  unset($set_var);
  var_dump( is_set($scalar_var, $set_var) );
}

echo "\n*** Testing unset(), empty() & is_set() with arrays ***\n";
...
foreach ($array_variables as $array_var) {
  ...
  // check the is_set and unset on non existing key
  $var = 1;  // a var which is defined
  // try to unset the element which is non-existent
  unset($array_var['non_existent']);
  // check using is_set() & empty() on a non_existent element in the array
  var_dump( is_set($array_var['non_existent']) );
  var_dump( is_set($array_var['non_existent'], $var) );
  var_dump( is_set($array_var['non_existent'], $array_var['none']) );
  var_dump( empty($array_var['non_existent']) );

  // testing empty and is_set on arrays
  var_dump( empty($array_var) ); // expecting bool(false), except: array(), which is considered empty
  var_dump( is_set($array_var) ); // expecting bool(true), except: array(), which is not set

  // get the keys of the $array_var
  $keys = array_keys($array_var);
  // unset each element in the array and see the working of unset, is_set & empty
  ...
  foreach ($keys as $key_value) {
    ...
    var_dump($array_var);
    // check using is_set for the element that was unset
    var_dump( is_set($array_var[$key_val]) ); // expected: bool(false)
    // calling is_set with more args
    var_dump( is_set($array_var[$key_val], $array_var) ); //expected: bool(false)
    ...
    // because of using is_set, empty and unset on its element
    var_dump($array_var);
  }
  ...
  var_dump($array_var);
  // use is_set to see that array is not set
  var_dump( is_set($array_var) ); //expected: bool(false)
  var_dump( is_set($array_var, $array_var[$key_val]) ); // expected: bool(false)

  // empty() to see if the array is empty
  var_dump( empty($array_var) ); // expected: bool(true)
}

echo "\n*** Testing unset(), emtpy() & is_set() with resource variables ***\n";
$fp = fopen(__FILE__, "r");
...
foreach ($resources as $resource) {
  ...
  // check using is_set() and empty()
  var_dump( is_set($resource) );  // expected: bool(true)
  var_dump( empty($resource) );  // expected: bool(false)
  // call is_set() with two args, both set
  var_dump( is_set($resource, $temp_var) ); // expected: bool(true)

  // dump the resource to see using is_set() and empty () had no effect on it
  var_dump($resource);

  // unset the resource
  unset($resource);
  // check using is_set() and empty()
  var_dump( is_set($resource) );  // expected: bool(flase)
  var_dump( empty($resource) );  // expected: bool(true)
  // call is_set() with two args, but one set
  var_dump( is_set($resource, $temp_var) ); // expected: bool(false)
  // uset the temp_var
  unset($temp_var);
  // now the is_set() with both the args as unset
  var_dump( is_set($resource, $temp_var) ); // expected: bool(false);
...
unset($resources);
var_dump($resources);
var_dump( is_set($resources) );  //expected: bool(false)
var_dump( empty($resources) );  // expected: bool(true)

echo "\n*** Testing unset(), empty() & is_set() with objects ***\n";
class Point
{
  var $x;
  var $y;
...
var_dump($point1);
var_dump( is_set($point1->x) );  // expected: bool(false)
...
var_dump($point1);
var_dump( is_set($point1) );  // expected: bool(ture)
var_dump( empty($point1) );  // expected: bool(false)

//unset the object and check
unset($point1);
var_dump( is_set($point1) );  // expected: bool(false)
var_dump( empty($point1) );  // expected: bool(true)
// dump to see that object is unset
var_dump($point1);

// try is_set/unset/empty on a member function
$point2 = new Point(5,6);
var_dump( is_set($point2->testPoint) );
...
/* testing variation in operation for is_set(), empty() & unset().
...
echo "\n*** Testing possible variation in operation for is_set(), empty() & unset() ***\n";
...
function test_unset2( &$ref_val ) {
  ...
  unset($ref_val);
  // check using is_set and empty to confirm
  var_dump( is_set($ref_val) );
...
function test_unset3() {
  ...
  // check the $global_var using is_set and empty
  var_dump( is_set($global_var) );
...
===DONE===
--EXPECTF--
*** Testing unset(), empty() & is_set() with scalar variables ***
-- Iteration 1 --
bool(true)
bool(true)
bool(true)
