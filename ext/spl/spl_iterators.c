...
next_step:
		...
		switch (object->iterators[object->level].state) {
			...
			case RS_CHILD:
				...
				if (!ce || !instance_of_function(ce, spl_ce_RecursiveIterator TSRMLS_CC)) {
					if (child) {
						zval_ptr_dtor(&child);
					}
...
static void spl_recursive_it_it_construct(INTERNAL_FUNCTION_PARAMETERS, zend_class_entry *ce_base, zend_class_entry *ce_inner, recursive_it_it_type rit_type)
{
	...
	switch(rit_type) {
		case RIT_RecursiveTreeIterator: {
			...
			if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "o|lzl", &iterator, &flags, &user_caching_it_flags, &mode) == SUCCESS) {
				if (instance_of_function(Z_OBJCE_P(iterator), zend_ce_aggregate TSRMLS_CC)) {
	...
	if (!iterator || !instance_of_function(Z_OBJCE_P(iterator), spl_ce_RecursiveIterator TSRMLS_CC)) {
...
static spl_dual_it_object* spl_dual_it_construct(INTERNAL_FUNCTION_PARAMETERS, zend_class_entry *ce_base, zend_class_entry *ce_inner, dual_it_type dit_type)
{
	...
	switch (dit_type) {
		...
		case DIT_IteratorIterator: {
			...
			if (!instance_of_function(ce, zend_ce_iterator TSRMLS_CC)) {
				if (ZEND_NUM_ARGS() > 1) {
					if (zend_lookup_class(class_name, class_name_len, &pce_cast TSRMLS_CC) == FAILURE
					|| !instance_of_function(ce, *pce_cast TSRMLS_CC)
				...
				if (instance_of_function(ce, zend_ce_aggregate TSRMLS_CC)) {
					...
					if (!retval || Z_TYPE_P(retval) != IS_OBJECT || !instance_of_function(Z_OBJCE_P(retval), zend_ce_traversable TSRMLS_CC)) {
...
static inline void spl_limit_it_seek(spl_dual_it_object *intern, long pos TSRMLS_DC)
{
	...
	if (pos != intern->current.pos && instance_of_function(intern->inner.ce, spl_ce_SeekableIterator TSRMLS_CC)) {
		MAKE_STD_ZVAL(zpos);
		ZVAL_LONG(zpos, pos);
		spl_dual_it_free(intern TSRMLS_CC);
