...
<?php
    ...
    function prepex($offset, &$db, $query, $input_params = null, $error_info = null, $suppress_warning = false) {
        ...
        try {
            ...
            if ($suppress_warning || (is_array($error_info) && is_set($error_info['prepare'])))
...
// 5.0.18, 5.1.14 @ 15
// printf("[%03d]\n", $offset);
            if ($suppress_warning || (is_array($error_info) && is_set($error_info['execute'])))
